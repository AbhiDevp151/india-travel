...
import axios from 'axios'; // Axios import kiya database call ke liye

const UserProfile = () => {
  const navigate = useNavigate();
  const [user, setUser] = useState(null);
  const [bookings, setBookings] = useState([]); // Ab ye empty array se start hoga
  const [loadingBookings, setLoadingBookings] = useState(true); // Loading state add ki
  ...
  useEffect(() => {
    ...
    if (!storedUser) {
      ...
      setUser(parsedUser);
      fetchMyBookings(parsedUser.id || parsedUser._id); // Page load hote hi bookings le aao
    }
  }, [navigate]);

  // Backend se Bookings lane ka function
  const fetchMyBookings = async (userId) => {
    try {
      const res = await axios.get(`/api/bookings/user/${userId}`);
      setBookings(res.data); // Database ka data seedha state mein
    } catch (error) {
    ...
    <main className="min-h-screen bg-zinc-950 text-white pt-24 pb-32 font-sans selection:bg-orange-500 relative">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-12">
          ...
          {/* LEFT: Profile Card (Same as before) */}
            ...
            <div className="bg-zinc-900/80 backdrop-blur-2xl border border-white/10 rounded-[3rem] p-8 text-center shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <div className="w-full space-y-3 mb-8">
                  <div className="bg-white/5 border border-white/5 rounded-2xl p-4 flex justify-between items-center text-left">
                    <div>
                      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Phone</p>
                      <p className="font-bold text-sm">{user.phone || 'Not Added'}</p>
                    </div>
                    <button className="text-orange-500 hover:text-white transition-colors text-xs font-black uppercase">Edit</button>
                  </div>
                </div>

                <button className="w-full bg-white/5 hover:bg-orange-500 border border-white/10 hover:border-orange-500 text-white font-black py-4 rounded-2xl uppercase tracking-widest transition-all text-xs">
                  Update Profile
                </button>
              </div>
            </div>
          ...
          <div className="lg:col-span-8">
            <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/5 rounded-[3rem] p-8 md:p-12 h-full shadow-2xl">
              ...
              {loadingBookings ? (
                <div className="text-center py-20 text-gray-400">Fetch kar raha hoon bhai...</div>
              ) : (
                      ...
                      <div className="flex-1 w-full">
                        <div className="flex justify-between items-start mb-2">
                          <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest">
                            ID: {booking._id.substring(0, 8).toUpperCase()} {/* ID ko chhota karke dikha rahe hain */}
                          </p>
                          <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full ${booking.status === 'Confirmed' ? 'bg-green-500/20 text-green-500 border border-green-500/20' : 'bg-gray-500/20 text-gray-400 border border-gray-500/20'}`}>
                            {booking.status}
                          </span>
                        </div>
